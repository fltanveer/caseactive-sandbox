        ...
        <div className="modal-overlay" onClick={onClose}>
            <div className="ccm afm-modal" onClick={e => e.stopPropagation()}>
                <div className="ccm-header">
                    <div>
                        <p className="ccm-breadcrumb">Library · Task Templates</p>
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="ccm afm-modal" onClick={e => e.stopPropagation()}>
                <div className="ccm-header">
                    <div>
                        <p className="ccm-breadcrumb">Library · Task Templates · {shortId(row.id)}</p>
                        <h2 className="ccm-title">Edit Task</h2>
                    </div>
                    <button className="ccm-close" onClick={onClose}><CloseIcon /></button>
                </div>
                <div className="ccm-body">
                    ...
                    <div className="vtm-left">
                        {row.description && (
                            <div className="vtm-section">
                                ...
                                <div className="vtm-desc rte-content" dangerouslySetInnerHTML={{ __html: row.description }} />
                            </div>
                        )}
...
/* ── Edit Task Modal (editable: title, RTE description, attachments, RTE child tasks w/ their own attachments, assignees) ── */
const EditTaskModal = ({ row, onClose, onSave }) => {
    const initPerms = (arr) =>
        (arr || []).map(a => PERMISSION_OPTIONS.find(o => o.toLowerCase() === a.toLowerCase())).filter(Boolean);
    ...
    const [subtasks, setSubtasks]     = useState(
        row.subtasks.map(s => ({ id: s.id, title: s.title, description: s.description || '', attachments: s.attachments || [] }))
    );
    ...
    const addSubtask = () => {
        ...
        setSubtasks(prev => [...prev, { id, title: '', description: '', attachments: [] }]);
        setExpanded(prev => new Set(prev).add(id));
    };
    const updateSubtaskField = (id, field, val) =>
    ...
    const handleSubtaskAttach = (id, e) => {
        const picked = Array.from(e.target.files || []);
        setSubtasks(prev => prev.map(s => s.id === id ? { ...s, attachments: [...s.attachments, ...picked.map(f => f.name)] } : s));
        e.target.value = '';
    };
    ...
    const save = () => {
        ...
        onSave(row.id, {
            title,
            description,
            permissions: perms.map(p => p.toLowerCase()),
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="vtm-modal" onClick={e => e.stopPropagation()}>
                <div className="vtm-header">
                    <div className="vtm-header-left">
                        <span className="vtm-breadcrumb">Library · Task Templates · {shortId(row.id)}</span>
                ...
                <div className="vtm-body">
                                                    ...
                                                    <div className="vtm-subtask-detail">
                                                        <div className="vtm-subtask-detail-field">
                                                            ...
                                                            <RichTextEditor
                                                                ...
                                                                onChange={val => updateSubtaskField(st.id, 'description', val)}
                    ...
                    <div className="vtm-right">
                        <div className="vtm-section">
                            <p className="vtm-section-label">Who can be assigned? <span className="ccm-req">*</span></p>
                            <MultiSelect options={PERMISSION_OPTIONS} value={perms} onChange={setPerms} placeholder="Select assignee type" allValue="All" />
                        </div>
...
const TaskTemplatesView = ({ addOpen = false, onCloseAdd }) => {
    ...
    return (
        <div className="cases-view">
            ...
            {editTaskRow && (
                <EditTaskModal row={editTaskRow} onClose={() => setEditTaskRow(null)} onSave={saveSettings} />
            )}
            {deleteTarget && (
                <DeleteConfirmModal
                    ...
                    <div key={r.id} className="tt-table-row">
                        ...
                        <span data-label="Action">
                            <span className="ft-action-wrap">
                                ...
                                {r.published === false ? (
                                    <button className="ft-icon-btn" title="Edit Task" onClick={() => setEditTaskRow(r)}>
